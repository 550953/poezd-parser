...
//         echo "<pre>";
//     	print_r(json_decode($arr_station));
//      	echo "<pre>";
//     }
}
...
if($trains != null && $full_name != null && $dateStart != null) {
    ...
    $query = "SELECT * FROM `rx_train_info` WHERE `number`='".$trains."' AND `date_start` = '".$dateStart."'";
...
function getFromBase($trains, $date, $link){
    ...
    $query = "SELECT * FROM `rx_train_info` WHERE `number`='".$trains."' AND `date_start` = '".$date."'";
    $result = mysqli_query($link,$query);
    if($result){
        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
            $id_train = $row['id'];
...
function getPartMarshrut($startSt, $endst, $arrStation){
	...
	foreach($arrStation as $station){
    	if($station['name'] == $startSt){
            echo $station['name']." name rec";
        	$record = true;
        }
    	if($station['name'] == $endst){
            echo $station['name']." name stop";
        	$record = false;
        }
    	if($record){
        	$result_arr[] = $station;
...
function SaveDateTrainInfo($stationArray, $trains, $fullTrain, $date, $link){
    ...
    foreach ($stationArray['stations'] as $st){
    	...
    	$id_side = SaveTrainSide($trains, $id_station, $link);
    	$query_side = "SELECT * FROM `rx_train_side` WHERE `id` = '".$id_side."'";
    	$result_side = mysqli_query($link,$query_side);
    	if($result_side){
        	while ($row = mysqli_fetch_array($result_side, MYSQLI_ASSOC)) {
            	$side_none = $row['none_side'];
            	$side_left = $row['left_side'];
            	$side_right = $row['right_side'];
           		$side_over = $row['over_side'];
    	...
    	$query_st = "SELECT * FROM `rx_station_info` WHERE `id` = '".$id_station."'";
    	...
    	$query = "INSERT INTO `rx_train_route` (`id`, `id_train`, `id_station`, `prib`, `wait`, `otpr`, `dist`, `day`, `vagon`, `diffTime`, `position`) VALUES (NULL, '".$id_train."', '".$id_station."', '".$st['prib']."','".$st['wait']."','".$st['otpr']."','".$st['dist']."','".$st['day']."','0','".$st['diffTime']."', '".$i."')";
    
       // $query = "INSERT INTO `rx_train_station`(`id`, `id_train`, `name`, `code`, `prib`, `wait`, `otpr`, `dist`, `day`,`vagon`,`diffTime`) VALUES
       // (null,'".$id_train."','".$st['name']."','".$st['code']."','".$st['prib']."','".$st['wait']."','".$st['otpr']."','".$st['dist']."','".$st['day']."','0','".$st['diffTime']."')";
        mysqli_query($link, $query);
   		$i++;
    }
    $j = 0;
    ...
    foreach ($stationArray['vagon'] as $st){
    	...
    	$query = "INSERT INTO `rx_train_route` (`id`, `id_train`, `id_station`, `prib`, `wait`, `otpr`, `dist`, `day`, `vagon`, `diffTime`, `position`) VALUES (NULL, '".$id_train."', '".$id_station."', '".$st['prib']."','".$st['wait']."','".$st['otpr']."','".$st['dist']."','".$st['day']."','1','".$st['diffTime']."', '".$j."')";
    
        //$query = "INSERT INTO `rx_train_station`(`id`, `id_train`, `name`, `code`, `prib`, `wait`, `otpr`, `dist`, `day`,`vagon`) VALUES
       //(null,'".$id_train."','".$st['name']."','".$st['code']."','".$st['prib']."','".$st['wait']."','".$st['otpr']."','".$st['dist']."','".$st['day']."','1','".$st['diffTime']."')";
        mysqli_query($link, $query);
    $j++;
    }
