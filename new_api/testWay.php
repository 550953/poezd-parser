...
//     $result_arr = getTR($_GET['trains'], $_GET['date']);
//     echo "<pre>";
//     print_r($result_arr);
//      echo "<pre>";
...
if($trains != null && $full_name != null && $dateStart != null) {
    ...
    if($count > 0){
        ...
        if($result > 86400){
            ...
            if(isset($result_arr['stations']) && $result_arr['stations'] != null)
...
foreach ($stationArray['stations'] as $st){
	...
	if(!file_exists($filename)){
        ...
        if($count > 0){
        	while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
            	$id_sound = $row['id'];
            	$query_sound = "UPDATE `not_sound` SET `code`='".$st['code']."',`name`='".$st['name']."' WHERE `id`='".$row['id']."'";
...
function hasSoundCode($code, $name,$link){
	...
	if(!file_exists($filename)){
        ...
        if($count > 0){
        	while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
            	$id_sound = $row['id'];
            	$query_sound = "UPDATE `not_sound` SET `code`='".$code."',`name`='".$name."' WHERE `id`='".$row['id']."'";
            	mysqli_query($link, $query_sound);
        	}
        }else{
        	$query_sound = "INSERT INTO `not_sound` (`id`, `code`, `name`) VALUES (NULL,'".$code."','".$name."')";
...
function SaveStation($name, $code, $link){
	$query = "SELECT * FROM `rx_station_info` LEFT JOIN `station_list` ON rx_station_info.code = station_list.code WHERE rx_station_info.code = '".$code."'";
    $result = mysqli_query($link,$query);
	$count = mysqli_num_rows($result);
