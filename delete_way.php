...
{
    $query = "SELECT * FROM `rx_train_way` WHERE `date_create` < DATE_SUB(NOW(), INTERVAL 10 DAY) LIMIT 1";
  ...
  //  echo $query;
    if ($result) {
        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
            ...
            // $queryStDel2 = "DELETE FROM `rx_train_station` WHERE `id_train` <= ".$id;
            // mysqli_query($link, $queryStDel2);
            
            // $querySt2 = "SELECT * FROM `rx_train_station` WHERE `id_train` <= ".$id;
            // $resultSt2 = mysqli_query($link, $querySt2);
            // if ($resultSt2) {
                
            //     while ($rowSt2 = mysqli_fetch_array($resultSt2, MYSQLI_ASSOC)) {
